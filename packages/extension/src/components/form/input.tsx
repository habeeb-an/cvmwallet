...
import { FormFeedback, FormGroup, FormText, InputGroup, Input as ReactStrapInput, Label } from 'reactstrap';
...
export const Input = forwardRef<HTMLInputElement, InputProps & React.InputHTMLAttributes<HTMLInputElement>>(
  (props, ref) => {
    const { type, label, text, error, append, styleInputGroup, typeInput, classnameinputgroup } = props;
    ...
    return (
      <FormGroup>
        ...
        <InputGroup style={styleInputGroup} className={classnameinputgroup}>
          <ReactStrapInput
            id={inputId}
            className={classnames('form-control-alternative', props.className, styleInput.input)}
            type={typeInput ?? type}
            innerRef={ref}
            {...attributes}
          />
