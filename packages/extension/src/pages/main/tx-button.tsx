...
export const TxButtonView: FunctionComponent<TxButtonViewProps> = observer(({ setHasSend, hasSend }) => {
  ...
  const hasAssets = queryBalances.balances.find((bal) => bal?.balance?.toDec().gt(new Dec(0))) !== undefined;
...
export const TxButtonEvmView: FunctionComponent<TxButtonViewProps> = observer(({ setHasSend, hasSend }) => {
  ...
  const hasAssets = isTronNetwork
    ...
    : parseFloat(evmBalance?.trim(true).shrink(true).maxDecimals(6).toString()) > 0;
...
export const TxButtonBtcView: FunctionComponent<TxButtonViewProps> = observer(({ setHasSend, hasSend }) => {
  const { accountStore, chainStore, queriesStore, keyRingStore } = useStore();
