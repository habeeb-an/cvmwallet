...
export const AddAddressModal: FunctionComponent<{
  ...
  typeAddress?: string;
  chainId: string;
}> = observer(({ closeModal, recipientConfig, memoConfig, addressBookConfig, index, typeAddress }) => {
  const intl = useIntl();
  ...
  return (
    <>
      ...
      <form className={style.formAdd}>
        ...
        />
        <MemoInput memoConfig={memoConfig} label={intl.formatMessage({ id: 'setting.address-book.memo' })} />
        <div
          style={{
            display: 'flex'
          }}
